...
func DecideRelayRetry(c *gin.Context, err *types.NewAPIError, retryTimes int) PolicyDecision {
	...
	if action, ok := MatchChannelErrorAction(err); ok {
		...
		case operation_setting.ChannelErrorActionRetryNext:
			return PolicyDecision{Action: "retry", Reason: "channel_error_rule", Source: "channel_error_rule"}
		case operation_setting.ChannelErrorActionPassthrough:
			return PolicyDecision{Action: "stop", Reason: "channel_error_rule_passthrough", Source: "channel_error_rule"}
		}
