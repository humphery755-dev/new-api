...
func taskPluginProtocolHandlers(protocol, operation string) ([]gin.HandlerFunc, error) {
	...
	case "openai_responses.create":
		return []gin.HandlerFunc{
			...
			middleware.ModelRequestRateLimit(), middleware.ConcurrencyQueue(), middleware.PinTaskPluginEndpoint(), middleware.PrepareTaskPluginEndpoint(), middleware.Distribute(),
			func(c *gin.Context) {
	...
	case "openai_video.create":
		return []gin.HandlerFunc{
			...
			middleware.PinTaskPluginEndpoint(), middleware.TaskPluginEndpointOnly(middleware.ModelRequestRateLimit()), middleware.ConcurrencyQueue(), middleware.PrepareTaskPluginEndpoint(), middleware.Distribute(),
